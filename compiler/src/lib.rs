    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            Self::InvalidImport(token) => {
                ...
                write!(f, "لا يمكن الاستيراد من داخل الدوال\n{token}")
            }
            Self::ImportParserError(token, errors) => {
                for err in errors {
                    writeln!(f, "{err}")?;
                }
                write!(f, "{token}")
            ...
            Self::InvalidExport(token) => {
                write!(f, "لا يمكن التصدير من داخل الدوال\n{token}")
            }
...
impl<'a> Compiler<'a> {
    ...
    fn import_path(&mut self, token: Rc<Token>) -> Result<ImportPath, ()> {
        use ImportPathType::*;
    ...
    fn for_in_stml(
        ...
        self.define_variable(Rc::new(Token::default()))?;
    ...
    pub fn compile(&mut self) -> Result<Function, Vec<CompileError>> {
