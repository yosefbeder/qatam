...
use compiler::value::{Arity, ArityType::*, Closure, Function, Native, Object, UpValue, Value};
...
impl fmt::Display for RuntimeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", format!("خطأ تنفيذي: ").bright_red())?;
        writeln!(f, "{}", self.value.to_string())?;
        writeln!(f, "{}", self.backtrace.frames[0].token)?;
...
impl<'a, 'b> compiler::value::Frame for Frame<'a, 'b> {
    ...
    fn nth_i32(&self, idx: usize) -> Result<i32, Value> {
        if let Value::Number(n) = self.nth(idx) {
            if n.fract() == 0.0 {
                Ok(*n as i32)
            } else {
    ...
    fn nth_u32(&self, idx: usize) -> Result<u32, Value> {
        if let Value::Number(n) = self.nth(idx) {
    ...
    fn nth_string(&self, idx: usize) -> Result<&str, Value> {
    ...
    fn nth_char(&self, idx: usize) -> Result<char, Value> {
        if let Value::Object(Object::String(string)) = self.nth(idx) {
    ...
    fn nth_object(&self, idx: usize) -> Result<&Rc<RefCell<HashMap<String, Value>>>, Value> {
    ...
    fn nth_list(&self, idx: usize) -> Result<&Rc<RefCell<Vec<Value>>>, Value> {
    ...
    fn nth_file(&self, idx: usize) -> Result<&Rc<RefCell<File>>, Value> {
    ...
    fn nth_path(&self, idx: usize) -> Result<PathBuf, Value> {
